...
@Component({
    selector: 'jhi-job-notification',
    templateUrl: './job-notification.component.html'
})
export class Job_notificationComponent implements OnInit, OnDestroy {

    job_notifications: Job_notification[];
    currentAccount: any;
    ...
    loadAll() {
        if (this.currentSearch) {
            this.job_notificationService.search({
        ...
        this.job_notificationService.query({
            page: this.page,
            size: this.itemsPerPage,
            sort: this.sort()
        }).subscribe(
    ...
    private onSuccess(data, headers) {
        ...
        for (let i = 0; i < data.length; i++) {
            this.job_notifications.push(data[i]);
        }
    }
