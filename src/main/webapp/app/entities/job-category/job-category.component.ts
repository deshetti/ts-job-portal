...
export class Job_categoryComponent implements OnInit, OnDestroy {
    ...
    loadAll() {
        if (this.currentSearch) {
            this.job_categoryService.search({
        ...
        this.job_categoryService.query({
            page: this.page,
            size: this.itemsPerPage,
            sort: this.sort()
        }).subscribe(
    ...
    trackId(index: number, item: Job_category) {
        return item.id;
    }
    registerChangeInJob_categories() {
        this.eventSubscriber = this.eventManager.subscribe('job_categoryListModification', (response) => this.reset());
    }
    ...
    private onSuccess(data, headers) {
        ...
        for (let i = 0; i < data.length; i++) {
            this.job_categories.push(data[i]);
        }
    }
